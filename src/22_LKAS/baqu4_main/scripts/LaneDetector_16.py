...
rospy.init_node('detect+steering', anonymous=True)
...
def setup_path():
    path = "./source/WIN_20220925_14_42_31_Pro.mp4"
...
def linear_regs(img, left, right, isRedCorn):
    ...
    if np.isnan(cross_x)==False and np.isnan(cross_y)==False:
        ...
        if 80<x<110:
            vanishAngle = speed2angle(w(outer_control(steering_vanishing_point(cross_x, WIDTH))), spd)
            print("소실점 조향 서보모터 각도: ", vanishAngle)
            pub.publish(vanishAngle)
        else:
            linearAngle = speed2angle(w(outer_control(x)), spd)
            print("기울기 조향 서보모터 각도: ", linearAngle)
            pub.publish(linearAngle)
...
def draw_circle(img, points, color:tuple):
    for p in points:
        cv2.circle(img, (p[0],p[1]), 10, color, -1, cv2.LINE_AA)
...
def callback2(data):
    ...
    print(spd)
    #os.system('clear')
    print('속도 : ' + str(int(spd)))

rospy.Subscriber("webcam_image", Image, callback)
rospy.Subscriber("RPM", Float32, callback2)
