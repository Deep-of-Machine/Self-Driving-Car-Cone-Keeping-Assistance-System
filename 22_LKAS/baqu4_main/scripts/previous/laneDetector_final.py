...
lower_yellow = (19-4, 50, 30) # hsv 이미지에서 바이너리 이미지로 생성 , 적당한 값 30
upper_yellow = (19+10, 255, 255)
...
##    작업공간    ##
    ...
    for r in r_mid:
        cv2.circle(img, (r[0], r[1]), 10, (0, 255, 255), -1, cv2.LINE_AA) #노랑
    for l in l_mid:
        cv2.circle(img, (l[0], l[1]), 10, (255, 0, 0), -1, cv2.LINE_AA) #파랑
    
    if isUseableRed==False and isRedValueable(s_mid, p_r_m, p_r_n, p_l_m, p_l_n):
        isUseableRed=True

    if isUseableRed and p_l_m!=0 and p_r_m!=0:
